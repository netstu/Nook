# Java Env Wrapper Phase 4 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add `Env.isInstanceOf(obj, klass)` in a Frida-aligned way while preserving the corrected Android attach lifetime model from earlier Env phases.

**Architecture:** Keep `Env` runtime-backed and narrow. Parse the first argument as a Java object wrapper and the second as a `Java.use(...)` class wrapper, then perform the actual JNI `IsInstanceOf(...)` call while a local `JavaEnv jenv` is alive at the call site on Android.
...
### Task 1: Add failing desktop tests for `Env.isInstanceOf(obj, klass)`
...
- `env.isInstanceOf(obj, klass)` returns a boolean
- the JNI helper receives the Java object handle and target class name
- non-Java-object first input is rejected
- non-class-wrapper second input is rejected

Use the existing fake Java object wrapper and class-wrapper styles.
...
Do not add string overloads or pointer/class-ref second arguments in this phase.

**Step 4: Commit**

Skip commit in-session unless explicitly requested.

### Task 2: Implement the narrow host test hook for instance checking
...
### Task 3: Implement `Env.isInstanceOf(obj, klass)` in the runtime
...
For Android, ensure the actual JNI `IsInstanceOf(...)` call executes while a local `JavaEnv jenv` is alive.
...
- a valid Java object wrapper as the first argument
- a valid Java class wrapper as the second argument
...
- phase-1, phase-2, and phase-3 Env tests remain green
...
### Task 4: Add an Android smoke for phase 4

**Files:**
- Create: `host/nook-py/java_env_wrapper_phase4_smoke.js`
...
- positive instance result for `instance` vs its own class wrapper
- negative instance result for `instance` vs a different class wrapper

Keep it focused and avoid string overloads or class-pointer arguments.
...
```powershell
nook-cli attach com.demo.target -l E:\Learn\my_program\all_my_hook\kanxue\Nook\host\nook-py\java_env_wrapper_phase4_smoke.js --wait --usb
```

Expected:

- own-class result is `true`
- different-class result is `false`
...
**Step 1: Document the phase-4 surface**

Record:

- why `isInstanceOf(...)` follows `isSameObject(...)`
- why the second argument is limited to Java class wrappers
