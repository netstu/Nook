...
typedef void (*HookEngineCallback)(HookContext* ctx, void* user_data);
...
typedef struct HookEntry {
    ...
    HookEngineCallback on_enter;    /* Enter callback (for attach mode) */
    HookEngineCallback on_leave;    /* Leave callback (for attach mode) */
 ...
 * @return              0 on success, -1 on failure
 */
int hook_attach(void* target, HookEngineCallback on_enter, HookEngineCallback on_leave, void* user_data, int stealth);
...
void* hook_create_redirect(uint64_t key, void* original_entry,
                           HookEngineCallback on_enter, void* user_data);
 ...
 */
void* hook_create_native_trampoline(uint64_t key, HookEngineCallback on_enter, void* user_data);
...
uint64_t hook_art_router_lookup(uint64_t original);
 ...
 */
void* hook_replace(void* target, HookEngineCallback on_enter, void* user_data, int stealth);
