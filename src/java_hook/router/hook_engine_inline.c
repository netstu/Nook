...
void emit_callback_call(Arm64Writer* w, HookEngineCallback callback, void* user_data) {
...
void emit_replace_epilogue(Arm64Writer* w) {
    /* Restore d0-d7 so replace/native thunks can override FP return values. */
    for (int i = 0; i < 8; i += 2) {
...
void* generate_attach_thunk(HookEntry* entry, HookEngineCallback on_enter,
                                    HookEngineCallback on_leave, void* user_data,
 ...
 */
static void* generate_replace_thunk(HookEntry* entry, HookEngineCallback on_enter,
