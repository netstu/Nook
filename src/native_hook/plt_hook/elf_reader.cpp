...
int ElfReader::hook(const char *func_name, void *new_func, void **old_func) {
    ...
    if (0 == findSymbolByName(func_name, &sym, &symidx)) {
        ...
        rel = this->pltRel;
        ElfHooker::RelocationCandidate relocation = {};
        for (uint32_t i = 0; i < this->pltRelCount; i++) {
            relocation.symbol_index = ELF_R_SYM((rel + i)->r_info);
            relocation.relocation_type = ELF_R_TYPE((rel + i)->r_info);
            relocation.relocation_offset = static_cast<uintptr_t>((rel + i)->r_offset);
            if (ElfHooker::FindFirstMatchingRelocationOffset(&relocation,
                                                             1u,
                                                             symidx,
                                                             R_GENERIC_JUMP_SLOT,
                                                             R_GENERIC_JUMP_SLOT,
...
#else
            LOGD("%08llx %08llx %-35s %08llx %s", (unsigned long long)r_offset, (unsigned long long)r_info, NULL!=map ? map->name : "Unknown", (unsigned long long)sym->st_value, name);
#endif
        }
    }
...
int ElfReader::hookInternally(void *addr, void *new_func, void **old_func) {
    ...
    prot |= PROT_WRITE;
    ElfW(Addr) target = reinterpret_cast<ElfW(Addr)>(addr);
    const ElfHooker::PatchPageRange patch_range =
            ElfHooker::ComputePatchPageRange(static_cast<uintptr_t>(target), sizeof(void*), PAGE_SIZE);
    void *start = reinterpret_cast<void *>(patch_range.start);
    if (0 == patch_range.start || 0 == patch_range.length) {
        LOGE("failed to compute patch range for %p in %s", addr, this->moduleName);
        return -1;
    }
    const int original_prot = PFLAGS_TO_PROT(phdr->p_flags);
    if (0 != mprotect(start, patch_range.length, prot)) {
        LOGE("failed to mprotect %p in %s", start, this->moduleName);
        return -1;
    }
    if (!ElfHooker::CaptureAndWritePointer(reinterpret_cast<void **>(addr), new_func, old_func)) {
        mprotect(start, patch_range.length, original_prot);
        LOGE("failed to write patched pointer for %p in %s", addr, this->moduleName);
        return -1;
    }

    LOGD("plt addr = %p",addr);
    ElfHooker::clear_cache(start, patch_range.length);
    if (0 != mprotect(start, patch_range.length, original_prot)) {
        LOGE("failed to restore mprotect %p in %s", start, this->moduleName);
        return -1;
    }
    return 0;
}
