...
void PrintUsage(const char* program) {
    std::cerr << "Usage: " << program << " <pid> [timeout_ms] [host] [port]\n";
}
...
int main(int argc, char** argv) {
    ...
    nook::comm::ResumeRequest request;
    request.pid = pid;

    nook::comm::ResumeResponse response;
    std::string error_message;
    if (!client.Resume(timeout_ms, request, &response, &error_message)) {
        std::cerr << "resume failed: " << error_message << "\n";
        return ExitWithCleanup(3);
    }

    std::cout << "resume ok: pid=" << response.pid << "\n";
