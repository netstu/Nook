...
int main(int argc, char** argv) {
    ...
    if (!client.EnumerateProcesses(timeout_ms, &response, &error_message)) {
        std::cerr << "process list failed: " << error_message << "\n";
        return ExitWithCleanup(3);
    }

    std::cout << "process count: " << response.processes.size() << "\n";
