param(
    ...
    [string]$DeviceServerScript = ".\tools\device_start_nook_server.ps1"
)
...
if ($StartNookServer) {
    $serverArgs = @(
        ...
        "-File", (Join-Path $repoRoot "tools\device_start_nook_server.ps1")
    )
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "device server start failed"
    }
}
...
& powershell @args
if ($LASTEXITCODE -ne 0) {
    throw "TargetDemo connect validation failed"
}
